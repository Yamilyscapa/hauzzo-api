...
router.get('/mine', auth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const brokerId = req.userId || ''
    const search = typeof req.query.q === 'string' ? req.query.q : undefined
    const { data, error } = await listLeadsForBroker(brokerId, search)
    if (error) {
      errorResponse(res, typeof error === 'string' ? error : 'Error fetching leads', 400, error)
      return
    }
